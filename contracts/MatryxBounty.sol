...
contract MatryxBounty is Ownable
{
    ...
    * - Finalized: After the round has completed
    */
    ...
    function submit(bytes _submission, address _payout) canSubmit(currentRound) payable {
        require(msg.value == entryFee);
        MatryxRound thisRound = MatryxRound(rounds[_roundNum]);
        thisRound.submit.value(msg.value)(_submission, _payout);
    }

    function pay(address _submitter) onlyOwner canPay(currentRound) {
    ...
    function refund() canRefund(currentRound) {
        MatryxRound r = MatryxRound(rounds[_roundNum]);
        r.refund();
    }

    function rate(address _submitter, uint256 rating) onlyOwner canPay(currentRound) {
        MatryxRound r = MatryxRound(rounds[_roundNum]);
        r.rate(_submitter, rating);
    }

    function closeRound(address _winner, uint256 _totalRating) onlyOwner canPay(currentRound) {
        MatryxRound r = MatryxRound(rounds[_roundNum]);
        currentRound++;
